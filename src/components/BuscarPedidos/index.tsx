...
const PedidoComponent = ({ pedido, reloadPedido }: { pedido: Pedido, reloadPedido: () => void }) => {
    return (
        <tr>
            {keysOfPedidos.map(key => key === 'situacao'
                ? <td key={`${pedido.pedidoBling}-${key}`}><PedidoSituation situacao={pedido.situacao} onChange={(e) => {
...
const BuscarPedidos = () => {
    ...
    return (
        <section id="buscarPedidos" className="flex items-center justify-center min-h-[30rem] h-[100vh] w-full ">
